...
void run_jack(bool async, size_t n_inputs, size_t buffer_size, int timeout = 0) {
    ...
    for (size_t i = 0; i < n_inputs; i++) {
        ...
        vertex = graph->add(IGpuFx::createBiquadEQ({
                                IBiquadParam::create(BiquadType::PEAK, 250, -7, 3),
                                IBiquadParam::create(BiquadType::PEAK, 500, -3, 1),
                                IBiquadParam::create(BiquadType::PEAK, 1000, 2, 1),
                                IBiquadParam::create(BiquadType::PEAK, 2000, -2, 1),
                                IBiquadParam::create(BiquadType::PEAK, 4000, 1, 1),
                            }),
                            vertex);
