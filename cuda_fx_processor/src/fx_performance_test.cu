...
void stream_vs_graph(size_t process_buffer_size, size_t n_warmup, size_t n_measure, bool simulate_buffer_intervals, bool process_in_place) {
    ...
    test(measurements, IGpuFx::createConv2c2(IPCMSignal::readFromFile(path::ir("vocal-duo-48k-24b-2c.wav")), 1 << 17, 0, 1.0f), process_buffer_size, n_warmup, n_measure, simulate_buffer_intervals, process_in_place);
    test(measurements, IGpuFx::createConv2c1(ir_pcm->clone(), 1 << 17, 0, 1.0f), process_buffer_size, n_warmup, n_measure, simulate_buffer_intervals, process_in_place);
    test(measurements, IGpuFx::createConv1c1(ir_pcm->clone(), 1 << 17, 0, 1.0f), process_buffer_size, n_warmup, n_measure, simulate_buffer_intervals, process_in_place);
...
void stream_all(size_t process_buffer_size, size_t n_warmup, size_t n_measure, bool simulate_buffer_intervals, bool process_in_place) {
    std::vector<PerformanceMeasurement*> measurements;
    IPCMSignal* ir_pcm = IPCMSignal::readFromFile(path::ir("vocal-duo-48k-24b-1c.wav"));
    // testStream(measurements, IGpuFx::createConv2c2(IPCMSignal::readFromFile(path::ir("vocal-duo-48k-24b-2c.wav")), 1 << 17, 0, 1.0f), process_buffer_size, n_warmup, n_measure, simulate_buffer_intervals, process_in_place);
    // testStream(measurements, IGpuFx::createConv2c1(ir_pcm->clone(), 1 << 17, 0, 1.0f), process_buffer_size, n_warmup, n_measure, simulate_buffer_intervals, process_in_place);
    // testStream(measurements, IGpuFx::createConv1c1(ir_pcm->clone(), 1 << 17, 0, 1.0f), process_buffer_size, n_warmup, n_measure, simulate_buffer_intervals, process_in_place);
...
void graph_all(size_t process_buffer_size, size_t n_warmup, size_t n_measure, bool simulate_buffer_intervals, bool process_in_place) {
    std::vector<PerformanceMeasurement*> measurements;
    IPCMSignal* ir_pcm = IPCMSignal::readFromFile(path::ir("vocal-duo-48k-24b-1c.wav"));
    // testGraph(measurements, IGpuFx::createConv2c2(IPCMSignal::readFromFile(path::ir("vocal-duo-48k-24b-2c.wav")), 1 << 17, 0, 1.0f), process_buffer_size, n_warmup, n_measure, simulate_buffer_intervals, process_in_place);
    // testGraph(measurements, IGpuFx::createConv2c1(ir_pcm->clone(), 1 << 17, 0, 1.0f), process_buffer_size, n_warmup, n_measure, simulate_buffer_intervals, process_in_place);
    // testGraph(measurements, IGpuFx::createConv1c1(ir_pcm->clone(), 1 << 17, 0, 1.0f), process_buffer_size, n_warmup, n_measure, simulate_buffer_intervals, process_in_place);
