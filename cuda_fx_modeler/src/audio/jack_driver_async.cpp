...
class SignalChainMappingAsync {
   public:
    SignalChainMappingAsync(ISignalGraph* signal_chain, std::vector<std::string> input_port_names, std::vector<std::string> output_port_names)
...
class JackDriverAsync : public Driver {
   private:
    ...
    static int processCallback(jack_nframes_t n_samples, void* arg) {
        assert(arg);
        auto jc = static_cast<JackDriverAsync*>(arg);
    ...
    void process(size_t n_samples) {
        ...
        for (SignalChainMappingAsync chain_map : _signal_chain_mappings) {
            ...
            chain_map.signal_chain->processAsync(dst_bufs, src_bufs);
        }
    }
   ...
   public:
    ...
    void start() {
        ...
        for (SignalChainMappingAsync chain_map : _signal_chain_mappings) {
            ...
            chain_map.signal_chain->startProcessThread();
        }
    ...
    void stop() {
        ...
        for (SignalChainMappingAsync chain_map : _signal_chain_mappings) {
            chain_map.signal_chain->stopProcessThread();
        }
        _is_running = false;
        usleep(500000);
    }
    ...
    void addSignalChain(ISignalGraph* chain, std::vector<std::string> inputs, std::vector<std::string> outputs) {
        _signal_chain_mappings.push_back(SignalChainMappingAsync(chain, inputs, outputs));
...
Driver* Driver::createJackDriverAsync() {
    return new JackDriverAsync();
}